...
public class JCommanderTest {
	...
	String[] argv = { "-debug", "-log", "2", "-float", "1.2", "-double",
		"1.3", "-bigdecimal", "1.4", "-date", "2011-10-26", "-groups",
		"unit", "a", "b", "c" };
	...
	Assert.assertEquals(args.date,
		new SimpleDateFormat("yyyy-MM-dd").parse("2011-10-26"));
    }

    @DataProvider
    public Object[][] alternateNamesListArgs() {
	return new Object[][] {
		new String[][] { new String[] { "--servers", "1", "-s", "2",
			"--servers", "3" } },
		new String[][] { new String[] { "-s", "1", "-s", "2",
			"--servers", "3" } },
		new String[][] { new String[] { "--servers", "1", "--servers",
			"2", "-s", "3" } },
		new String[][] { new String[] { "-s", "1", "--servers", "2",
			"-s", "3" } },
		new String[][] { new String[] { "-s", "1", "-s", "2",
			"--servers", "3" } }, };
    }
    ...
    /**
     * Getting the description of a nonexistent command should throw an
     * exception.
     */
    @Test(expectedExceptions = ParameterException.class)
	...
	JCommander jc = new JCommander(new Object[] { argsHelp,
		new ArgsRequired() },
	...
	String fileName = "a";
	new JCommander(args, "-file", "/tmp/" + fileName, "-listStrings",
		"Tuesday,Thursday", "-listInts", "-1,8", "-listBigDecimals",
		"-11.52,100.12");
	...
	Assert.assertEquals(args.listBigDecimals.get(0), new BigDecimal(
		"-11.52"));
	Assert.assertEquals(args.listBigDecimals.get(1), new BigDecimal(
		"100.12"));
    }
	...
	jc = new JCommander(new Args1(), new String[] { "-debug", "-log", "2",
		"-long", "5" });
	...
	verifyCommandOrdering(new String[] { "add", "commit" }, new Object[] {
		new CommandAdd(), new CommandCommit() });
	verifyCommandOrdering(new String[] { "commit", "add" }, new Object[] {
		new CommandCommit(), new CommandAdd() });
    }

    @DataProvider
    public static Object[][] f() {
	return new Integer[][] { new Integer[] { 3, 5, 1 },
		new Integer[] { 3, 8, 1 }, new Integer[] { 3, 12, 2 },
	...
	Assert.assertEquals(
		...
		"Options: "
			+ EnumSet
				.allOf((Class<? extends Enum>) ArgsEnum.ChoiceType.class));
	...
	j.parse("-groups", "a,b", "-ints", "41,42", "-hp",
		"localhost:1000;example.com:1001", "-hp2",
	...
	{
	    String args[] = { "-paramA", "a1", "a2", "-paramB", "b1", "b2",
		    "b3" };
	    Params p = new Params();
	    new JCommander(p, args).parse();
	    Assert.assertEquals(p.paramA,
		    Arrays.asList(new String[] { "a1", "a2" }));
	    Assert.assertEquals(p.paramB,
		    Arrays.asList(new String[] { "b1", "b2", "b3" }));
	}

	{
	    String args[] = { "-paramA", "a1", "a2", "-paramB", "b1",
		    "-paramA", "a3" };
	    Params p = new Params();
	    new JCommander(p, args).parse();
	    Assert.assertEquals(p.paramA,
	...
	class Arg {
	    @Parameter(names = "-rule", description = "rule")
	    private List<String> rules = new ArrayList<String>();
	    @Parameter(names = "-other", description = "other")
	...
	Arg a = new Arg();
	JCommander jc = new JCommander(a);
	jc.parse("-rule", " some test", "-other", "other test ", " arg1",
		"arg2 ", " arg3 ");
	Assert.assertEquals(a.rules, Arrays.asList("some test"));
	Assert.assertEquals(a.other, Arrays.asList("other test"));
	Assert.assertEquals(a.args, Arrays.asList("arg1", "arg2", "arg3"));

	a = new Arg();
	jc = new JCommander(a);
	jc.setPreserveSpaces(true); // used to request to not trim args
	jc.parse("-rule", " some test", "-other", "other test ", " arg1",
		"arg2 ", " arg3 ");
	Assert.assertEquals(a.rules, Arrays.asList(" some test"));
	Assert.assertEquals(a.other, Arrays.asList("other test "));
	...
	Assert.assertEquals(V2.names,
		Arrays.asList(new String[] { "-h", "--host" }));
	...
	Assert.assertEquals(jc.getUnknownOptions(),
		Lists.newArrayList("-a", "foo"));
    }
	...
	new JCommander(a, new String[] { "-name", "theName", "--", "param1",
		"param2" });
	...
	Arguments a = new Arguments();
	new JCommander(a, new String[] { "param1", "param2", "--", "param3",
		"-name", "theName" });
